...
uniform sampler2D LutImageTexture;// lookup texture
in vec2 v_texCoord;
layout(location = 0) out vec4 outColor;

void main(){
    lowp vec4 color = texture2D(s_TextureMap, v_texCoord);
    ...
    lowp vec4 left = texture2D(LutImageTexture, vec2(u, v));

    u  = blockRight / 15.0 * 240.0;
    u  = (ceil(color.r * 15.0) / 15.0 * 15.0) + u;
    u /= 255.0;
    v  = (ceil(color.g * 15.0) / 15.0);
    lowp vec4 right = texture2D(LutImageTexture, vec2(u, v));
